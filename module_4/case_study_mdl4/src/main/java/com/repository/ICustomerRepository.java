...
public interface ICustomerRepository extends JpaRepository<Customer,Integer> {
 ...
 //   @Query(value = "select c.* from `customer` c join `customer_type` ct on c.customer_type_id = ct.id where c.name like concat ('%',:name,'%') and ct.name like ('%',:customerType,'%') and c.email like concat ('%',:email,'%')", nativeQuery = true)
    @Query(value = "select c.* from `customer` c join `customer_type` ct on c.customer_type_id = ct.id where c.name like %:name% and ct.name like %:customerType% and c.email like %:email%", nativeQuery = true)
    Page<Customer> searchName(@Param("name") String name, @Param("customerType") String customerType, @Param("email") String email, Pageable pageable);
