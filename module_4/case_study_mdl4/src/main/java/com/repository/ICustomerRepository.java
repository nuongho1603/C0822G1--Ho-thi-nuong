...
public interface ICustomerRepository extends JpaRepository<Customer,Integer> {
    ...
    @Query(value = "select c.* from `customer` c join `customer_type` ct on c.customer_type_id = ct.id where c.name like concat('%',:name,'%') and c.email like concat('%',:email,'%') and ct.name like %:customerType%", nativeQuery = true)
